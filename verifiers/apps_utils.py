...
def run_test(in_outs: dict = None, completion: str = None):
    """
    If test is not None it'll try to run the code.
    Otherwise, it'll just return an input and output pair.
    """
    assert completion is not None and in_outs is not None, "Test and input-outputs must be provided\n"
    ...
    for index, inputs in enumerate(in_outs["inputs"]):
        ...
        if which_type == CODE_TYPE.call_based:  # Call-based
            ...
            if isinstance(output, list):
                ...
                raise ValueError("Unsupported output type captured\n")
            ...
            try:
                tmp_result = tmp_result or (set(frozenset(round(float(t), 3) for t in s) for s in output) == set(
                    frozenset(round(float(t), 3) for t in s) for s in
                    in_outs["outputs"][index]))
            except Exception as e:
